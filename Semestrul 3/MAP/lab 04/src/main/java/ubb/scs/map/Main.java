...
public class Main {
    public static void main(String[] args) {
        ...
        var uR = new UserRepository(uV, Paths.get("resources", "users.txt").toString());

        var comm = new CommunityNetwork();

        var fV = new FriendshipValidator();
        var fR = new FriendshipRepository(fV, Paths.get("resources", "friendships.txt").toString(), comm);
