...
public class MySocialNetworkService implements SocialNetworkService<Long, User> {
    ...
    public MySocialNetworkService(UserRepository userRepo,
                                  ...
                                  FriendshipRequestDBRepository friendshipRequestRepo, MessageDBRepository messageRepo, NotificationRepository notificationRepo) {
    ...
    public void addFriendshipRequest(Long userId1, Long userId2) {
        try {
            ...
            User receiver = getUser(userId2);
            addNotification(new Notification(null, userId2, "Friend Request", "You have a new friend request from " + getUser(userId1).getFirstName() + " " + getUser(userId1).getLastName(), LocalDateTime.now()));
    ...
    public void sendMessage(Message message) {
        try {
            ...
            addNotification(new Notification(null, message.getTo(), "New Message", "You have a new message from " + getUser(message.getFrom()).getFirstName() + " " + getUser(message.getFrom()).getLastName(), LocalDateTime.now()));
    ...
    public void acceptFriendshipRequest(FriendshipRequest fr) {
        try {
            ...
            addNotification(new Notification(null, fr.getFirst(), "Friend Request Accepted", "Your friend request to " + getUser(fr.getSecond()).getFirstName() + " " + getUser(fr.getSecond()).getLastName() + " has been accepted.", LocalDateTime.now()));
            addNotification(new Notification(null, fr.getSecond(), "New Friend", "You are now friends with " + getUser(fr.getFirst()).getFirstName() + " " + getUser(fr.getFirst()).getLastName() + ".", LocalDateTime.now()));
    ...
    public List<User> getFriendsPage(Long userId, int pageNumber, int pageSize) {
        return friendshipRepo.getFriendsPage(userId, pageNumber, pageSize);
    }

    public int getTotalFriendsCount(Long userId) {
        return friendshipRepo.getTotalFriendsCount(userId);
    }

    @Override
    public void addNotification(Notification notification) {
        try {
            var max = streamOf(notificationRepo.findAll())
            ...
            throw new ServiceException("Error adding notification: " + e.getMessage());
        }
    }
    ...
    public void deleteNotification(Long notificationId) {
        try {
            ...
            throw new ServiceException("Error deleting notification: " + e.getMessage());
        }
    }
    ...
    public List<Notification> getNotifications(Long userId) {
        try {
            return StreamSupport.stream(notificationRepo.findAllForUser(userId).spliterator(), false)
                    .sorted(Comparator.comparing(Notification::getDateTime).reversed())
                    .toList();
        } catch (RepoException e) {
            throw new ServiceException("Error getting notifications: " + e.getMessage());
        }
    }
}