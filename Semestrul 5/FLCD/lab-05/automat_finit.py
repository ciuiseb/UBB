...
class AutomatFinit:
    ...
    def from_file(cls, filename):
        ...
        # --- CHANGED ---
        # Allow state names like 'Start', 'Hex_Prefix', 'Identifier', etc.
        state_name_pattern = r"[a-zA-Z_][a-zA-Z0-9_]*"

        # --- CHANGED ---
        # 1. Use new state_name_pattern for a single state
        state_regex = rf"({state_name_pattern})"

        # --- CHANGED ---
        # 2. Use new state_name_pattern for a list of states
        state_list_regex = rf"({state_name_pattern}(?:\s*,\s*{state_name_pattern})*)"

        # This regex for symbols seems to work for you, so we'll keep it
        symbol_char_class = r"[a-zA-Z0-9\.+\-bBxX]" # Added bBxX just in case
        ...
        if not match:
            ...
            print("Q: State1,State2,...;")
            print("Σ: a,b,...;")
            print("q0: State1;")
            print("F: State2,State3,...;")
            print("Tranzitii:")
            print("State1: (a,State2),(b,State1);")
            print("State2: (a,State3);")
            print("...;")
            return None
        ...
        try:
            ...
            state_transition_pattern = re.compile(
                rf"({state_name_pattern})\s*:\s*(.*?);", re.DOTALL
            )

            pair_pattern = re.compile(r"\(\s*({0})\s*,\s*({1})\s*\)".format(symbol_char_class, state_name_pattern))
