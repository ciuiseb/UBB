...
import {MapMarker, MultiMapModalComponent} from "../location/multi-map-modal.component";
...
export class HomePage implements OnInit, ViewWillEnter {
  ...
  async openMultiMapModal() {
    const backendUrl = '/api/all-books-locations';

    try {
      const booksWithLocations =  await (await this.bookService.findAllLocations()).toPromise();

      const validMarkers = (booksWithLocations || [])
        .filter(book => book.latitude && book.longitude) as MapMarker[];

      if (validMarkers.length === 0) {
        this.modalCtrl.create({
          component: MultiMapModalComponent,
          componentProps: {
            markers: [],
