...
public class BookController {
    ...
    public ResponseEntity<BookDTO> updateBook(@PathVariable Long id, @RequestBody BookDTO dto) {
        ...
        if (existingBookOpt.isPresent()) {
            ...
            book.setPhoto(dto.getPhoto());
            book.setLatitude(dto.getLatitude());
            book.setLongitude(dto.getLongitude());
    ...
    @GetMapping("/api/books/all-locations")
    public ResponseEntity<List<BookDTO>> getAllBookLocations(Principal principal) {
        String username = principal.getName();
        User user = userRepository.findByUsername(username).orElseThrow();
        List<Book> allBooks = bookRepository.findByUserId(user.getId().toString());
        List<BookDTO> booksWithLocations = allBooks.stream()
                .filter(book -> book.getLatitude() != null && book.getLongitude() != null)
                .map(BookDTO::toDto)
                .toList();

        return ResponseEntity.ok(booksWithLocations);
    }
}