...
import triathlon.services.ITriathlonClient;
import triathlon.services.ITriathlonServices;
import triathlon.services.messaging.MessageBrokerPublisher;
...
public class ServiceImpl implements ITriathlonServices {
    ...
    private final MessageBrokerPublisher broker;

    private Map<Long, ITriathlonClient> loggedReferees;
    ...
    public ServiceImpl(EventRepository eventRepository, ParticipantRepository participantRepository,
                       ...
                       RefereeDisciplineRepository refereeDisciplineRepository, MessageBrokerPublisher broker) {
    ...
    @Override
    public synchronized RefereeDiscipline login(Referee tempReferee, ITriathlonClient client) throws TriathlonException {
    ...
    @Override
    public synchronized void logout(Referee referee, ITriathlonClient client) throws TriathlonException {
        logger.info("Logout attempt from referee: " + referee.getUsername());

        ITriathlonClient localClient = loggedReferees.remove(referee.getId());
    ...
    private void notifyResultUpdated(Result result) {
        ...
        try {
            broker.publishResultUpdate(result);
        } catch (Exception e) {
            logger.error("Error publishing to message broker: {}", e.getMessage());
        }
    }
}
