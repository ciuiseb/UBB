...
public class TriathlonClientJsonWorker : ITriathlonObserver, IDisposable
{
    ...
    public void Run()
    {
        while (_connected)
        {
            try
            {
                ...
                {
                    // No data yet, just continue the loop
                    continue;
                }
                ...
                _logger.Info($"Request: {requestLine}");
            ...
            // Short pause between processing iterations
            try
            {
                Thread.Sleep(100); // Reduced from 1000ms to be more responsive
            }
    ...
    private void SendResponse(Response response)
    {
        ...
        string responseLine = JsonConvert.SerializeObject(response, settings);
        _logger.Info($"Sending response: {responseLine}");
